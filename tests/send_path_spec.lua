...
describe("send_path", function()
    ...
    it("should send multiple file paths on separate lines", function()
      ...
      if sent_content then
        for _, path in ipairs(paths) do
          assert.is_truthy(sent_content:find(path, 1, true), 
            "Expected path " .. path .. " to be in sent content")
        end
  ...
  describe("send_path integration", function()
    ...
    it("should open file picker and send selected paths", function()
      ...
      assert.is_truthy(sent_content:find("lua/tmux%-send/init%.lua", 1, false) or 
                       sent_content:find("/home/user/project/lua/tmux-send/init.lua", 1, true),
                       "Expected to find init.lua path in: " .. sent_content)
      assert.is_truthy(sent_content:find("test_file%.lua", 1, false) or 
                       sent_content:find("/home/user/project/tests/test_file.lua", 1, true),
